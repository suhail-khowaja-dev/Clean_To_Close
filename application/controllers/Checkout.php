        ...
        if (count($_POST) > 0) {
            ...
            if (isset($_POST['checkbox']) && $_POST['checkbox'] == 'on') {
                ...
                $result = Braintree_Transaction::sale(array(
                    ...
                    'merchantAccountId' => '',
                    'paymentMethodNonce' => $token, 'options' => array(
...
//
    ...
    public function stripe_payment()
    {
        ...
        $gettingMemeberAmount = $this->model_signup->find_one($params, true);
        $memberAmount = $gettingMemeberAmount->signup_members_amount;
        $package_amount = $this->input->post('amount');
        $final_amount = $package_amount + $memberAmount;
        // debug($final_amount,1);
        ...
        if ($response['status'] == 'succeeded') {
            $data1 = $this->model_packages->find_by_pk(1);
            $data2 = $this->model_packages->find_by_pk(2);
            $data3 = $this->model_packages->find_by_pk(3);

            if ($this->input->post('amount') == $data3['packages_amount']) {

                $package_jobs = $package_jobs + 5;
            }
            if ($this->input->post('amount') == $data2['packages_amount']) {

                $package_jobs = $package_jobs + 10;
            }
            if ($this->input->post('amount') == $data1['packages_amount']) {

                $package_jobs = $package_jobs + 25;
            }

            if (isset($_POST) and array_filled($_POST)) {
                $id = $this->input->post('id');
                $this->model_signup->update_by_pk($id, array(
                    'signup_name_card' => $this->input->post('name_card'),
                    'signup_address_card_associated'=>$this->input->post('address_card_associated'),
                    'signup_card_number' => $this->input->post('card-number'),
                    'signup_cvc_number' => $this->input->post('card-cvc'),
                    'signup_payement_status' => 1,
                    'signup_card_month' => $this->input->post('card-month'),
                    'signup_card_year' => $this->input->post('card-year'),
                    'signup_package_jobs' => $package_jobs,
                ...
                $datateam =  $this->session->userdata('insertedid');
                // $this->model_signup->auto_login($datateam);
                // redirect('account/user-login?type=2');
            }
            // $data = array(
            //     'signup_card_number' => $this->input->post('card-number'),
            ...
            // $this->db->where('signup_mls_id', $this->input->post('id'));

            // if($this->db->update('ctc_signup', $data))
            // {
            //    debug("heelo",1);
            // }
            // else
