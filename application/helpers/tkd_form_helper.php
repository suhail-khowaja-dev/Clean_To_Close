...
Class Tkd_form_helper
{
	...
	public function gen_update_timestamp($p=array())
	{
		...
		return '<input class=" form-control '.$class .'" id="'.$p['id'].'" '.$p['additional'].' name="' . $p['field_name'] .'" type="hidden" value="'.date("m-d-y g:i A").'"/>';
	}
...
<?=humanize($key).': '.$value?>
 ...
 //              </div>
	...
	public function gen_datetime($p=array())
	{
		...
		$p['field_data'] = ($p['field_data']=="0000-00-00" || !$p['field_data'] ) ?  date("m-d-y g:i A") : $p['field_data'] ;
		return '
