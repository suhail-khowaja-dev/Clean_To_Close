    ...
    <div class="portlet box green">
      ...
      <div class="portlet-body form">
        ...
        <form class="cmxform form-horizontal tasi-form" id="<?=(isset($form_id))?$form_id:'' ?>" method="POST" action="" enctype="multipart/form-data">
          ...
          <h3 class="form-section">Set Background Check Amount</h3>
            <div>
              <?foreach ($member_amount as $syscon) { ?>
                <div class="form-group ">
                      <label class="control-label col-md-2 " for="con<?=$syscon['member_amount_id']?>"> 
                          <?=humanize($syscon['member_amount_variable'])?>
                      </label>
                      <div class="col-md-9">
                        <input  type="text" name="member_amount_attr[<?=$syscon['member_amount_id']?>]"  value="<?=$syscon['member_amount_value']?>"  id="con<?=$syscon['member_amount_id']?>" class="form-control"/>
                      </div>
                </div>
              <?}?>  
            </div>
