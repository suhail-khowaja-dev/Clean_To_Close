    ...
    <div class="portlet box green">
      ...
      <div class="portlet-body form">
        ...
        <form class="cmxform form-horizontal tasi-form" id="<?=(isset($form_id))?$form_id:'' ?>" method="POST" action="" enctype="multipart/form-data">
          ...
          <h3 class="form-section">Set Email's</h3>
            <div>
              <?foreach ($emails as $syscon) { ?>
                <div class="form-group ">
                      <label class="control-label col-md-2 " for="con<?=$syscon['emails_id']?>"> 
                          <?=humanize($syscon['emails_variable'])?>
                      </label>
                      <div class="col-md-9">
                        <input  type="text" name="emails_attr[<?=$syscon['emails_id']?>]"  value="<?=$syscon['emails_value']?>"  id="con<?=$syscon['emails_id']?>" class="form-control"/>
                      </div>
                </div>
              <?}?>  
            </div>
