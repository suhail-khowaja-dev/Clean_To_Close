    ...
    <div class="portlet box green">
      ...
      <div class="portlet-body form">
        ...
        <form class="cmxform form-horizontal tasi-form" id="<?=(isset($form_id))?$form_id:'' ?>" method="POST" action="" enctype="multipart/form-data">
          ...
          <h3 class="form-section">Set Invoice Payment</h3>
            <div>
              <?foreach ($cleaner_invoice as $syscon) { ?>
                <div class="form-group ">
                      <label class="control-label col-md-2 " for="con<?=$syscon['cleaner_invoice_id']?>"> 
                          <?=humanize($syscon['cleaner_invoice_variable'])?>
                      </label>
                      <div class="col-md-9">
                        <input  type="text" name="cleaner_invoice_attr[<?=$syscon['cleaner_invoice_id']?>]"  value="<?=$syscon['cleaner_invoice_value']?>"  id="con<?=$syscon['cleaner_invoice_id']?>" class="form-control"/>
                      </div>
                </div>
              <?}?>  
            </div>
